...
test('admin auto-check previews ESPN outcomes, then applies locally before save',async({page})=>{
  ...
  const picks={Duncan:'K. Walker III Over 78.5 Rushing Yards',Jacob:'S. Barkley over 94.5 Rushing Yards',Matt:'D. Montgomery Over .5 Anytime TD',Weston:'J. Gibbs Over .5 Anytime TD'};
  for(const [player,pick] of Object.entries(picks))await page.locator(`textarea[name="leg-${player}"]`).fill(pick);
  await page.locator('textarea[name="single-desc"]').fill('Bills to Win vs Lions');
