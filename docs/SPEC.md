...
The $5 bet editor supports three structured bet types: winner by moneyline, winner against the spread, and game-total over/under. New structured $5 entries capture both the selected team and its opponent and present NFL teams by full team name rather than abbreviation. Canonical structured descriptions are human-readable, such as `Buffalo Bills to beat Detroit Lions`, `Buffalo Bills -3.5 vs Detroit Lions`, and `Buffalo Bills vs Detroit Lions over 54.5 total`. Legacy abbreviation-based or older supported descriptions remain readable/editable and are parsed into the same structured controls when possible; unsupported free text is preserved rather than discarded.
...
Supported player descriptions are rushing/receiving/passing yards, receptions, rushing/receiving/passing touchdowns, anytime TD, and interceptions, including their compact canonical aliases. Supported $5 team descriptions are moneyline/team winner, against-the-spread, and game-total over/under, including the full-name matchup-aware canonical formats and supported legacy aliases.
...
- Admin persistence fails closed: if the rendered admin is still in TEST MODE but the browser mode marker is transiently lost during a rerender/save race, the admin-only save wrapper restores TEST MODE state and keeps the write in isolated browser storage rather than falling through to production Supabase.
...
## 14. Source layout
`index.html` public shell; `admin/index.html` admin shell; `styles.css` shared base styling; `public.css` public-only branded dashboard styling; `public-team-bets.css` public $5 matchup/logo styling; `admin.css` admin-only styling; `js/config.js` config; `js/schema.js` initial data/normalization/validation; `js/calculations.js` business rules and completion-driven admin `currentWeek()`; `js/dashboard-week.js` Wednesday-noon-Central public calendar-week logic; `js/storage.js` shared Supabase/auth and TEST MODE persistence primitives; `js/storage-admin.js` admin-only fail-closed persistence routing for TEST MODE saves; `js/sleeper.js` Sleeper metadata/live reads/score import; `js/bet-entry.js` structured bet string builders/parsers, NFL team metadata/logo URLs, compact canonicalization, and ESPN roster normalization; `js/admin-bet-entry.js` ESPN-backed admin structured-entry UI; `js/bet-results.js` ESPN NFL result parsing/evaluation; `js/public.js` public rendering; `js/admin.js` core admin UI; `js/admin-qol.js` admin dirty-state/draft/closeout/selector enhancements; `js/bet-adjustments-admin.js` PrizePicks UI and Week 17 season-end settlement; `js/bet-results-admin.js` result preview/apply workflow; `tests/unit/` deterministic business-rule regressions; `tests/e2e/` Playwright browser workflows; `.github/workflows/qa.yml` CI.
...
20. New structured $5 team entries must identify both teams; public logo presentation is derived from recognized team names and is never canonical league data.
